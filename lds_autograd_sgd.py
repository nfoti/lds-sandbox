...
if __name__ == "__main__":
    ...
    def logZ(params, Ybatch):
        ...
        return lds_logZ(Ybatch, A, C, Q, R, mu0, Q0)

    # gradient descent with no line search
    #grad_AQ = grad(objective)
    step_size = 1e-4
    batch_sz = 100
    for gd_iter in range(200):

        trial_inds = np.random.choice(Y.shape[0], size=batch_sz, replace=False)
        Ybatch = Y[trial_inds]

        def objective(params_flat):
            params = unflatten(params_flat)
            return -logZ(params, Ybatch) + penalty(params)

        grad_AQ = grad(objective)
        grad_flat = grad_AQ(params_flat) * Y.shape[0] / batch_sz
        params_new_flat = params_flat - step_size * grad_flat

        if np.linalg.norm(grad_flat) <= 1e-6:
            break

        #if gd_iter > 150:
        #    step_size = 1e-4 / (gd_iter - 150)

        params_flat = params_new_flat
        callback(params_flat)
        print('iter:', gd_iter, 'step-size:', step_size)#' -- grad_norm:', np.linalg.norm(grad_flat))
    A_est_full, L_Q_est = unflatten(params_flat)
    Q_est = np.dot(L_Q_est, L_Q_est.T)

    print('fine tuning')
    # fine tune the sgd solutions
    spoptions = dict(maxiter=25)
    res = minimize(objective, params_flat, method='L-BFGS-B',
                   jac=grad(objective),
                   callback=callback,
                   #tol=1e-6,
